...
const defs = [
  ...
  { id: "final", title: "Final", blurb: "The production stack: LQIP placeholder + cache-guarded fade, over pixel-perfect token widths, with an optional per-image crop.",
    options: { ...base, placeholder: "lqip", animation: true, pixelPerfect: true, crop: true } },
] as const satisfies readonly { id: string; title: string; blurb: string; options: Options }[];
